...
**Option A: Switch to LightDM (Recommended)**
```bash
...
#### 3. Wayland Session Conflicts
**Problem**: NVIDIA driver doesn't work well with Wayland in KDE Plasma.

**Solution**: Force X11 session
```bash
...
# 7. Switch to LightDM (recommended for stability)
sudo apt install lightdm
sudo systemctl disable sddm
sudo systemctl enable lightdm
...
# May need to switch from SDDM to LightDM
```
...
| Best Display Manager | LightDM | LightDM/SDDM | LightDM |
