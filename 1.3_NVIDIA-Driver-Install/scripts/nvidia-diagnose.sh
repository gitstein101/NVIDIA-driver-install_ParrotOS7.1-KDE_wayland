...
print_header "Wayland Configuration"
{
    ...
    if [ -f /etc/lightdm/lightdm.conf ]; then
        ...
        echo "No /etc/lightdm/lightdm.conf"
    fi

    if [ -d /etc/lightdm/lightdm.conf.d ]; then
        echo ""
        echo "--- /etc/lightdm/lightdm.conf.d/ ---"
        for conf in /etc/lightdm/lightdm.conf.d/*; do
            if [ -f "$conf" ]; then
                echo "--- $conf ---"
                cat "$conf"
            fi
    ...
    if [ -f /usr/local/bin/nvidia-lightdm-setup.sh ]; then
        echo ""
        echo "--- /usr/local/bin/nvidia-lightdm-setup.sh ---"
        cat /usr/local/bin/nvidia-lightdm-setup.sh
    fi
